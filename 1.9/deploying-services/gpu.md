...
1.  Install DC/OS using the [custom installation instructions](/docs/1.9/installing/custom/advanced/) with the `enable-gpu-isolation: 'true'` configuration parameter specified.
...
In this example, an app is deployed with GPUs that specifies a Docker container and [DC/OS Universal Container Runtime (UCR)](/docs/1.9/deploying-services/containerizers/).  You must use UCR to run a containerized application that uses GPUs. To use the Universal Container Runtime, set the container type to `MESOS`.

...
Every change in the definition of applications or groups in DC/OS is performed as a Marathon deployment. A [deployment](/1.9/usage/marathon/rest-api/#!/deployments/V2Deployments) is a set of actions, including:
...
- **Health checks** are specified in each application and are run against tasks. If health check fails for a task, DC/OS will replace the task. For more information, see the [documentation](/docs/1.9/usage/marathon/health-checks/).

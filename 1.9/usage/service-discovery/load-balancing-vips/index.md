...
DC/OS comes with an east-west load balancer that's meant to be used to enable multi-tier microservices architectures. It acts as a TCP Layer 4 load balancer, and it's tightly integrated with the kernel. The internal Layer 4 load balancer component is also known as [Minuteman](/docs/1.9/overview/architecture/components/#minuteman).
